	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(option != null && !option.equals("")) {
			if(category != null && !category.equals("")) {	
				map = service.getBoardList("rogue", category, option, keyword, page);
			}else {
				map = service.getBoardList("rogue", page, option, keyword);
			}
			
		}else {
			
			if(category != null && !category.equals("")) {
				map = service.getBoardList("rogue", category, page);
			}else {
				map = service.getBoardList("rogue", page);
			}		
		}
