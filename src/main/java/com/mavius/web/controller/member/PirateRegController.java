...
@WebServlet("/board/pirate/reg")
@MultipartConfig(
	    fileSizeThreshold = 1024*1024,
	    maxFileSize = 1024*1024*10, 
	    maxRequestSize = 1024*1024*10*5
	)
public class PirateRegController extends HttpServlet{
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		
		
		RequestDispatcher dispatcher = request.getRequestDispatcher("/member/board/target/job/pirate/reg.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String path = request.getServletContext().getRealPath("/upload/pirate");
//		String path = "C:/upload/archer";
		...
		Board board = new Board(title, content, id, "pirate", category);
